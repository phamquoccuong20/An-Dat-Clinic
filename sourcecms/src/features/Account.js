    ...
    if (_resp?.status == "success") {
      ...
      if (typeof _resp.response.data.errors.msg === "string") {
        ...
        return rejectWithValue(
          { status: "failure" },
          { msg: _resp.response.data.errors.msg }
        );
      } else {
        if (typeof _resp.response.data.errors[0].msg === "string") {
          ...
          return rejectWithValue(
            { status: "failure" },
            { msg: _resp.response.data.errors[0].msg }
          );
        } else {
          dispatch(handleErrors(_resp.response.data.errors));
          return rejectWithValue(
            { status: "error" },
            { errors: _resp.response.data.errors }
          );
        }
      }
    }
...
export const { logout, refresh, renewToken, updateProfile } =
